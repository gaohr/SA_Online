		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<div class="filePicker btn btn-small btn-danger">选择文件</div>';
	            	html += '						<input id="fileImage" type="file" size="24" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area">可将文件拖到此处</span>';
					html += '			</div>';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '				<div class="btns pull-left">';
		            html += '					<div class="upload_btn btn btn-small btn-primary">上传文件</div>';
		            html += '				</div>';
		            html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var delHtml = "";
				if(para.del){  // 显示删除按钮
					delHtml = '<span class="file_del" data-index="'+file.index+'" title="删除"><i class="icon-remove"></i></span>';
				}
