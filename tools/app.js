...
// 	 ipobj=$.ajax({url:"http://192.168.205.44:8090/webproject/CatchIP",async:false});	
//  	 LocalhostIP = ipobj.responseText;
//  });
//    LocalhostIP = "159.226.110.183:" + port + "/";
  LocalhostIP = "192.168.205.44:" + port + "/";
//  LocalhostIP = "localhost";

var App = function () {
	
	//check cookies
	username_i = $.cookie("user_SA"); // 读取 cookie;
    //alert(username_i);
	if (username_i != null && username_i != "") {
		$("#username-in").html("" + username_i + "");
		$("#userhead-in").attr("src","../img/icon/monkey.png");
		$("#userout").hide();
	} else {
		$("#username-out").html("<a href='login/Login.html' style='color:#aaa'>登陆/注册</a>");
		$("#userhead-out").attr("src","../img/icon/user-b.png");
		$("#userin").hide();
	}
